...
public class WReadLock implements WLock {
	...
	protected WReadLock(String lockkey, LockPolicy lockPolicy, WLockClient wlockClient) {
	...
	private LockResult getReleaseLockResult(long lockversion) {
		...
		// 释放锁释放的是读锁还是写锁
		return this.wlockClient.getLockService().releaseLock(lockkey, lockversion, false, threadID,LockTypeEnum.readWriteReentrantLock.getValue(), ReadWriteLockTypeEnum.Read.getOpcode());
	}
