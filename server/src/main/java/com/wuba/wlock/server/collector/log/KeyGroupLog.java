...
public class KeyGroupLog extends CollectorLog {

    private static final Logger log = LoggerFactory.getLogger("KeyGroupLog");
            ...
            if (keyGroupQps != null && !keyGroupQps.isEmpty()) {
                ...
                for (Map.Entry<String, Map<Integer, QpsEntity>> entry: keyGroupQps.entrySet()) {
                    ...
                    if (groupQpsMap != null && !groupQpsMap.isEmpty()) {
                        List<Integer> groupIds = new ArrayList<Integer>(groupQpsMap.keySet());
                        Collections.sort(groupIds);
                        for (Integer groupId: groupIds) {
                            QpsEntity qpsEntity = groupQpsMap.get(groupId);
                            JSONObject jsonObject = new JSONObject();
                            jsonObject.put(nowMinue + "-" + key + "-" + groupId, qpsEntity);
