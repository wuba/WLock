...
public class WriteLockService extends BaseReadWriteLockService {
	private static Logger LOGGER = LoggerFactory.getLogger(WriteLockService.class);
	...
	public boolean tryAcquireLock(LockContext lockContext, int groupId) {
		...
		if (lock.isPresent() && !lock.get().isFree()) {
			...
			if (lockOwnerInfo != null && !lockOwnerInfo.isExpire() && lockOwnerInfo.equals(acquireLockRequest.getHost(), acquireLockRequest.getThreadID(), acquireLockRequest.getPid())) {
				...
				if (propose.getResult() == PaxosTryCommitRet.PaxosTryCommitRet_OK.getRet() && lockSmCtx.getLockRet() == LockResult.SUCCESS) {
					...
					traceWorker.offer(new LockTrace(TimeUtil.getCurrentTimestamp(), ProtocolType.ACQUIRE_LOCK, IPUtil.getIpStr(acquireLockDO.getHost()), acquireLockDO.getThreadID(),
							acquireLockRequest.getPid(), acquireLockRequest.getLockKey(), lockSmCtx.getFencingToken(), acquireLockRequest.getRegistryKey(), lockSmCtx.getExpireTime(), LockCodeEnum.Write_lock));
		...
		if (canGetWriteLock(lock, acquireLockRequest, groupId)) {
			SMCtx ctx = createCtx();
			byte[] proposeMsg;
			try {
				AcquireLockDO acquireLockDO = AcquireLockDO.fromRequest(acquireLockRequest, version);
	...
	private boolean canGetWriteLock(Optional<ReentrantLockValue> lock, AcquireLockRequest acquireLockRequest, int groupId) {
		...
		if (!lockOwnerInfo.isExpire()) {
			...
			if (propose.getResult() == PaxosTryCommitRet.PaxosTryCommitRet_OK.getRet() && lockSmCtx.getLockRet() == LockResult.SUCCESS) {
				...
				traceWorker.offer(new LockTrace(TimeUtil.getCurrentTimestamp(), ProtocolType.RELEASE_LOCK, IPUtil.getIpStr(lockOwnerInfo.getIp()), lockOwnerInfo.getThreadId(),
						lockOwnerInfo.getPid(), releaseLockRequest.getLockKey(), reentrantLockValue.getLockVersion(), releaseLockRequest.getRegistryKey(), -1, LockCodeEnum.Write_lock));
			...
			lockNotify.lockNotifyExpired(key, new LockOwner(lockOwnerInfo.getIp(), lockOwnerInfo.getThreadId(), lockOwnerInfo.getPid()), groupId, EventType.WRITE_LOCK_EXPIRED);
		...
		if (!lockOwnerInfo.isExpire()) {
			...
			if (propose.getResult() == PaxosTryCommitRet.PaxosTryCommitRet_OK.getRet() && lockSmCtx.getLockRet() == LockResult.SUCCESS) {
				...
				traceWorker.offer(new LockTrace(TimeUtil.getCurrentTimestamp(), ProtocolType.RENEW_LOCK, IPUtil.getIpStr(lockOwnerInfo.getIp()), lockOwnerInfo.getThreadId(),
						lockOwnerInfo.getPid(), renewLockRequest.getLockKey(), reentrantLockValue.getLockVersion(), renewLockRequest.getRegistryKey(), lockSmCtx.getExpireTime(), LockCodeEnum.Write_lock));
	...
	public boolean tryDeleteLock(DeleteLockRequest deleteLockRequest, int groupId) {
		...
		if (lockOwnerInfo.isExpire()) {
			...
			lockNotify.lockNotifyExpired(key, new LockOwner(lockOwnerInfo.getIp(), lockOwnerInfo.getThreadId(), lockOwnerInfo.getPid(), version), groupId, EventType.WRITE_LOCK_EXPIRED);
			SMCtx ctx = createCtx();
			ProposeResult result = proposeDeleteKey(deleteLockRequest, groupId, ctx, lockOwnerInfo, LockTypeEnum.readWriteReentrantLock.getValue(), OpcodeEnum.ReadWriteOpcode.WRITE.getValue());
			trySnatchLock(key, groupId, version, deleteLockRequest.getRegistryKey());
			LockSmCtx lockSmCtx = (LockSmCtx) ctx.getpCtx();
	...
	public boolean tryGetLock(LockContext lockContext, int groupId) {
		...
		try {
			...
			if (!lockOwnerInfo.isExpire()) {
				...
				ackGetLock(lockContext.getChannel(), getLockRequest, null, ResponseStatus.LOCK_DELETED);
				lockNotify.lockNotifyExpired(key, new LockOwner(lockOwnerInfo.getIp(), lockOwnerInfo.getThreadId(), lockOwnerInfo.getPid(), version), groupId, EventType.WRITE_LOCK_EXPIRED);
				proposeDeleteKey(getLockRequest, groupId, lockOwnerInfo, LockTypeEnum.readWriteReentrantLock.getValue(), OpcodeEnum.ReadWriteOpcode.WRITE.getValue());
				trySnatchLock(key, groupId, version, getLockRequest.getRegistryKey());
				return false;
			}
		} catch (LockException e) {
