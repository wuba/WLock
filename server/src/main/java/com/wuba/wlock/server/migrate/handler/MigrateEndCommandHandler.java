...
public class MigrateEndCommandHandler extends BaseCommandHandler {
    private static final Logger log = LoggerFactory.getLogger(MigrateEndCommandHandler.class);
    ...
    public boolean handle(MigrateCommandDO migrateCommandDO, MigrateSmCtx migrateSmCtx) {
        try {
            ...
            if (groupMigrateState == null) {
                ...
                log.error("MigrateEndCommandHandler group: {} groupMigrateState is null", migrateCommandDO.getSourceGroupId());
                return true;
            }

            int state = groupMigrateState.getState();
            if (MigrateType.MigrateGroupMovingSafePoint.getValue() != state && MigrateType.MigrateEnd.getValue() != state) {
                setMigrateResult(migrateSmCtx, MigrateResult.STATE_CHECK_FAIL);
                log.error("MigrateEndCommandHandler group: {} state not right. state: {}", migrateCommandDO.getSourceGroupId(), state);
                return true;
            }
            ...
            if (version != migrateCommandDO.getVersion()) {
                ...
                log.error("MigrateEndCommandHandler group: {} version not equals. version: {} commandVersion: {}",
                        migrateCommandDO.getSourceGroupId(), version, migrateCommandDO.getVersion());
                return true;
            }

            // 变更分组
            MigrateService.getInstance().migrateEnd(migrateCommandDO.getSourceGroupId());

            ClientManager.getInstance().removeLockClient(migrateCommandDO.getRegistryKey(), migrateCommandDO.getSourceGroupId());

            uploadMigrateState(migrateCommandDO);
        } catch (Exception e) {
            setMigrateResult(migrateSmCtx, MigrateResult.EXCEPTION);
            log.error("MigrateEndCommandHandler error", e);
            return false;
        }

        setMigrateResult(migrateSmCtx, MigrateResult.SUCCESS);
        log.error("MigrateEndCommandHandler execute success! migrateCommandDO: {}", JSON.toJSONString(migrateCommandDO));
        return true;
    }
}
