...
public class GroupChangeSafetypointCommandHandler extends BaseCommandHandler {
    private static final Logger log = LoggerFactory.getLogger(GroupChangeSafetypointCommandHandler.class);
    ...
    public boolean handle(MigrateCommandDO migrateCommandDO, MigrateSmCtx migrateSmCtx) {
        try {
            ...
            if (groupMigrateState == null) {
                ...
                log.error("GroupChangeSafetypointCommandHandler group: {} groupMigrateState is null", migrateCommandDO.getSourceGroupId());
                return true;
            }

            int state = groupMigrateState.getState();
            if (MigrateType.MigrateGroupStartMoving.getValue() != state && MigrateType.MigrateGroupMovingSafePoint.getValue() != state) {
                setMigrateResult(migrateSmCtx, MigrateResult.STATE_CHECK_FAIL);
                log.error("GroupChangeSafetypointCommandHandler group: {} state not right. state: {}", migrateCommandDO.getSourceGroupId(), state);
                return true;
            }
            ...
            if (version != migrateCommandDO.getVersion()) {
                ...
                log.error("GroupChangeSafetypointCommandHandler group: {} version not equals. version: {} commandVersion: {}",
            ...
            if (!registerKey.equals(migrateCommandDO.getRegistryKey())) {
                ...
                log.error("GroupChangeSafetypointCommandHandler group: {} registerKey not equals. registerKey: {} commandRegisterKey: {}",
                        migrateCommandDO.getSourceGroupId(), registerKey, migrateCommandDO.getRegistryKey());
                return true;
            }

            if (MigrateType.MigrateGroupStartMoving.getValue() == state) {
            ...
            // 变更分组
            MigrateService.getInstance().changeSafetypoint(migrateCommandDO.getSourceGroupId());

            uploadMigrateState(migrateCommandDO);
        } catch (Exception e) {
            setMigrateResult(migrateSmCtx, MigrateResult.EXCEPTION);
            log.error("GroupChangeSafetypointCommandHandler error", e);
            return false;
        }

        setMigrateResult(migrateSmCtx, MigrateResult.SUCCESS);
        log.error("GroupChangeSafetypointCommandHandler execute success! migrateCommandDO: {}", JSON.toJSONString(migrateCommandDO));
        return true;
    }
}
