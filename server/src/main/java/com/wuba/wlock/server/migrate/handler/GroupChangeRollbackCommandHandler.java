...
public class GroupChangeRollbackCommandHandler extends BaseCommandHandler {
    private static final Logger log = LoggerFactory.getLogger(GroupChangeRollbackCommandHandler.class);
    ...
    public boolean handle(MigrateCommandDO migrateCommandDO, MigrateSmCtx migrateSmCtx) {
        try {
            ...
            if (groupMigrateState == null || groupMigrateState.isDel()) {
                ...
                log.error("GroupChangeRollbackCommandHandler group: {} groupMigrateState is null", migrateCommandDO.getSourceGroupId());
                return true;
            }
            ...
            if (MigrateType.MigrateGroupStartMoving.getValue() != state) {
                setMigrateResult(migrateSmCtx, MigrateResult.STATE_CHECK_FAIL);
                log.error("GroupChangeRollbackCommandHandler group: {} state not right. state: {}", migrateCommandDO.getSourceGroupId(), state);
                return true;
            }
            ...
            if (version != migrateCommandDO.getVersion()) {
                ...
                log.error("GroupChangeRollbackCommandHandler group: {} version not equals. version: {} commandVersion: {}",
                        migrateCommandDO.getSourceGroupId(), version, migrateCommandDO.getVersion());
                return true;
            }

            // 变更分组
            MigrateService.getInstance().changeGroupRollback(migrateCommandDO.getSourceGroupId());
            ...
            log.error("GroupChangeRollbackCommandHandler error", e);
            return false;
        }

        setMigrateResult(migrateSmCtx, MigrateResult.SUCCESS);
        log.error("GroupChangeRollbackCommandHandler execute success! migrateCommandDO: {}", JSON.toJSONString(migrateCommandDO));
        return true;
    }
}
